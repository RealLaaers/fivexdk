RegisterNetEvent('pure-scripts:qb-clothing:migrate', function(skinTable)
    local citizenIDtoApply = skinTable.citizenid 
    local data = json.decode(skinTable.skin)
    local model = skinTable.model
    if not model then
        return
    end
