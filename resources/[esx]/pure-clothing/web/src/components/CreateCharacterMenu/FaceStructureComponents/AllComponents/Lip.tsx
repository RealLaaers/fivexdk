...
import { setThickness } from '../../../../store/features/createChar/lipSlice';

const Lip: React.FC = () => {
  const dispatch = useAppDistpatch();
  const state = useAppSelector((state) => state.lip);
  const config = useAppSelector((state) => state.config.config);

  if (!config.nui['sections']['faceStructure'].lip) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      >
        <h1>{language.menus.createCharacter.lipTitle}</h1>
        <p>
          <svg
            version="1.1"
            x="0px"
        ...
        <div className="sliderRow">
          ...
          <Slider
            ...
            onChange={(e: any, value, activeThumb) => {
              handleSliderChange(value, 'lip-thinkness');
            }}
          />
        </div>
      </ThemeProvider>
