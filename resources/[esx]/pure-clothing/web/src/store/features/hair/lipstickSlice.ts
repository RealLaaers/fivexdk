import { createSlice, PayloadAction } from '@reduxjs/toolkit';
import { sendNui } from '../../../utils/sendNui';

export interface LipstickState {
  baseValue1: number;
  maxValue1: number;
  colourID1: number;
  colourID2: number;
  opacity: number;
}

const initialState: LipstickState = {
  baseValue1: 0,
  maxValue1: 10,
  colourID1: 1,
  colourID2: 1,
  opacity: 100,
};

export const lipstickSlice = createSlice({
  name: 'lipstick',
  initialState,
  reducers: {
    setLipstickForFirstTime: (state, action: PayloadAction<number>) => {
      state.baseValue1 = action.payload;
    },
    setLipstickBaseValue1: (state, action: PayloadAction<number>) => {
      if (action.payload == 0) {
        state.baseValue1 = 0;
        return;
      }
      if (action.payload > 0) {
        if (state.baseValue1 + action.payload < state.maxValue1) {
          state.baseValue1 += action.payload;
        } else {
          state.baseValue1 = state.maxValue1;
        }
      } else {
        if (state.baseValue1 + action.payload > -1) {
          state.baseValue1 += action.payload;
        } else {
          state.baseValue1 = -1;
        }
      }
      sendNui('setAppearance', {
        name: 'lipstick',
        value: state.baseValue1,
        opacity: state.opacity,
      });
    },
    setLipstickBaseValue1ByInput: (state, action: PayloadAction<number>) => {
      if (action.payload > 0) {
        if (action.payload < state.maxValue1) {
          state.baseValue1 = action.payload;
        } else {
          state.baseValue1 = state.maxValue1;
        }
      } else {
        if (action.payload > -1) {
          state.baseValue1 = action.payload;
        } else {
          state.baseValue1 = -1;
        }
      }
      sendNui('setAppearance', {
        name: 'lipstick',
        value: state.baseValue1,
        opacity: state.opacity,
      });
    },
    setLipstickMaxValue1: (state, action: PayloadAction<number>) => {
      state.maxValue1 = action.payload;
    },
    setLipstickColourID1: (state, action: PayloadAction<number>) => {
      state.colourID1 = action.payload;
      sendNui('setColour', {
        name: 'lipstick',
        colour1: state.colourID1,
        colour2: state.colourID2,
      });
    },
    setLipstickColourID2: (state, action: PayloadAction<number>) => {
      state.colourID2 = action.payload;
      sendNui('setColour', {
        name: 'lipstick',
        colour1: state.colourID1,
        colour2: state.colourID2,
      });
    },
    setLipstickOpacity: (state, action: PayloadAction<number>) => {
      state.opacity = action.payload;
      sendNui('setAppearance', {
        name: 'lipstick',
        value: state.baseValue1,
        opacity: state.opacity,
      });
    },
  },
});

export default lipstickSlice.reducer;
export const {
  setLipstickBaseValue1,
  setLipstickBaseValue1ByInput,
  setLipstickMaxValue1,
  setLipstickColourID1,
  setLipstickColourID2,
  setLipstickOpacity,
  setLipstickForFirstTime,
} = lipstickSlice.actions;
