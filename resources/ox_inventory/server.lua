...
lib.addCommand({'additem', 'giveitem'}, {
	...
	if item then
		...
		if server.loglevel > 0 then
			lib.logger(source.owner, 'admin', ('"%s" gave %sx %s to "%s"'):format(source.label, count, item.name, inventory.label), "warn")
		end
	end
end)
...
lib.addCommand('removeitem', {
	...
	if item and args.count > 0 then
		...
		if server.loglevel > 0 then
			lib.logger(source.owner, 'admin', ('"%s" removed %sx %s from "%s"'):format(source.label, args.count, item.name, inventory.label), "warn")
		end
	end
end)
...
lib.addCommand('setitem', {
	...
	if item then
		...
		if server.loglevel > 0 then
			lib.logger(source.owner, 'admin', ('"%s" set "%s" %s count to %sx'):format(source.label, inventory.label, item.name, args.count), "warn")
		end
	end
end)
