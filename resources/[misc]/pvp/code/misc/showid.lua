...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsControlPressed(0, TGIANN.key) or forceDraw then
            if GetHashKey('WEAPON_UNARMED') ~= GetSelectedPedWeapon(ped) or triggered and Notif == false then
                exports['mythic_notify']:SendAlert('error', 'Tag våbenet væk, for at id peak!', "error")
                Citizen.Wait(2000)
            else
