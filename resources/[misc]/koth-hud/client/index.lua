...
AddEventHandler("UpdateInfo", function(stats)
  _teams[1].point = string.format("%03d", math.floor(stats.points[1]))
  _teams[2].point = string.format("%03d", math.floor(stats.points[2]))
  _teams[3].point = string.format("%03d", math.floor(stats.points[3]))
...
local point = lib.points.new(vec3(1530.6582, 1709.7032, 109.9309), 300)
local zone1 = false
...
local point2 = lib.points.new(vec3(-1629.3918, 209.7738, 60.6413), 300)
local zone2 = false
...
local point3 = lib.points.new(vec3(1078.0343, 2299.7446, 45.5086), 300)
local zone3 = false
...
function HUDPlayer()
  ...
  SendNUIMessage({ player = _PlayerInfo, showHud = true })
