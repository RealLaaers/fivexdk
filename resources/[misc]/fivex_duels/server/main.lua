...
local function handleNextMatchRound(match, finished, winner, cause)
    for i = 1, #match.team1, 1 do
        ...
        TriggerClientEvent("healffs2", match.team1[i].id)
        -- local groundZ = lib.callback.await('fivex_duels:client:getGroundCoords', match.team1[i].id, match.mapdata.coords.player[i])
        ...
        --     Wait(500)
        --     SetEntityCoords(ped, groundZ.x, groundZ.y,
        --     groundZ.z, false, false, false, false)
        -- end
    ...
    for i = 1, #match.team2, 1 do
        ...
        TriggerClientEvent("healffs2", match.team2[i].id)
        -- local groundZ = lib.callback.await('fivex_duels:client:getGroundCoords', match.team2[i].id, match.mapdata.coords.opponent[i])
...
lib.callback.register("duels:server:getRoutingBucket", function(source)
