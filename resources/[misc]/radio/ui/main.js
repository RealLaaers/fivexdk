...
function ChangeFunc() {
    if (radioWindow === 1) {
        $.post('https://radio/joinChannel', JSON.stringify({
            channel: $("#channel").val()
        }));
    } else if (radioWindow === 2) {
        $.post('https://radio/changeInfo', JSON.stringify({
            type: "name",
            value: $("#channel2").val()
        }));
...
function OnOff() {
    if (radioOn) {
        ...
        $("#radiolist").empty();
        $("#radioimage").attr("src","images/" + radioColour + (radioOn ? "/on_" + radioType : "/off") + ".png");
        $.post('https://radio/leaveChannel', JSON.stringify({}));
    } else {
        radioOn = true;
        $("#radioimage").attr("src","images/" + radioColour + (radioOn ? "/on_" + radioType : "/off") + ".png");
        $.post('https://radio/radioOn', JSON.stringify({}));
        $(".screen-" + radioWindow).css("display", "block");
...
function VolUp() {
    $.post('https://radio/VolUp', JSON.stringify({}));
    if (radioVolume < 100) {
...
function VolDown() {
    $.post('https://radio/VolDown', JSON.stringify({}));
    if (radioVolume > 10) {
        radioVolume = radioVolume - 10
        $("#curvolume").html("Volume: " + radioVolume)
    }
