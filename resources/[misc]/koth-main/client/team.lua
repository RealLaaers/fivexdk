...
AddEventHandler("Koth-Team", function(Team)
    ...
    if Team == "Rouge" then
        ...
        local zoneRouge = AddBlipForRadius(PosBaseRou.x,PosBaseRou.y,PosBaseRou.z, 25.0)
        ...
        local zoneBleu = AddBlipForRadius(PosBaseBle.x,PosBaseBle.y,PosBaseBle.z, 25.0)
        ...
        local zoneVer = AddBlipForRadius(PosBaseVer.x,PosBaseVer.y,PosBaseVer.z, 25.0)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if KOTH.Teams ~= 0 then
            ...
            if distance < 70.0 and not KOTH.InZone and not IsPlayerDead(PlayerId()) then
                TriggerServerEvent("PlayerEnteredKothZone",KOTH.TeamID)
                -- print("InZone")
                KOTH.InZone = true
            elseif (distance > 70.0 or IsPlayerDead(PlayerId())) and KOTH.InZone then
