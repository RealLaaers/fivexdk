...
RegisterCommand(Config.Leave_Training_Command, function()
    ...
    if Training == true then
        ...
        RemoveWeaponFromPed(Player_Ped, GetHashKey(GetHashKey(Training_Weapon)))
...
AddEventHandler('viber-aimlab:client:FinishMenu', function(name, photo, modename, modedif)
    if Leave == false then
        ...
        if modename == "Realistic Track" or modename == "Target Track" then
            ...
            ---------------------------------
            TriggerServerEvent('viber-aimlab:server:FinishTraining', 0, 0, Training_Temporary_Data.Shooted_Target*0.5)
        elseif modename == "Dynamic Clicking" then
            ...
            ---------------------------------
            TriggerServerEvent('viber-aimlab:server:FinishTraining', 0, 0, Training_Temporary_Data.Shooted_Target*Training_Target_Score)
        else
            SendNUIMessage({
                type = "finish-show",
                playername = name,
            ...
            ---------------------------------
            TriggerServerEvent('viber-aimlab:server:FinishTraining', Training_Temporary_Data.Shooted_Target, Training_Temporary_Data.Total_Target, Training_Temporary_Data.Shooted_Target*Training_Target_Score)
        end
    end
end)
