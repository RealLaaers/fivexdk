    ...
    openMainMenu = function()
        ...
        if crew then
            table.insert(elements, {
                    ...
                    description = 'Se oversigt over dit crew.',
                    icon = 'bars',
                    metadata = {
                      { label = 'Navn', value = crew.label or "N/A" },
                      { label = 'Tag', value = crew.tag or "N/A" },
                      { label = 'Medlemmer', value = tostring(utils.getMemberCount() or 0) },
                      { label = 'Points', value = tostring(crew.points or 0)},
                      { label = 'Kills', value = tostring(crew.kills or 0) },
                      { label = 'Deaths', value = tostring(crew.deaths or 0) },
                      { 
                        label = 'K/D', 
                        value = (crew.deaths and crew.deaths > 0) 
                                  and string.format("%.2f", crew.kills / crew.deaths) 
                                  or ((crew.kills and crew.kills > 0) and "∞" or "0.00")
                      },
                      { label = 'Dit Rank', value = shared.getRankLabel(crew.data[myIdentifier].Rank) or "N/A" },
                    },
            })

            table.insert(elements, {
                title = 'Medlems Oversigt',
                description = 'Se oversigt over dit crews medlemmer.',
                icon = 'users',
                onSelect = function()
        ...
        lib.registerContext({
            id = 'crew_menu',
            title = crew and ('Crew: ' .. crew.label) or _L('main_menu_title'),
            options = elements
        })
...
utils = {
    ...
    -- Definer getMemberCount direkte her:
    getMemberCount = function()
        if crew then
            local count = 0
            for _k, _v in pairs(crew.data) do
                count = count + 1
            end
            return count
        end
...
-- Hjælpefunktion til at tælle medlemmer i et crew
local function getCrewMemberCount(data)
    local count = 0
    for _k, _v in pairs(data or {}) do
        count = count + 1
    end
    return count
end
...
crewLeaderboard = {
    openLeaderboard = function()
        local allCrews = lib.callback.await('crews:getAllCrews', 5000)
        if not allCrews then
            notify("Kunne ikke hente leaderboard-data", "error")
            return
        end

        local sortedCrews = {}
        for _id, crewData in pairs(allCrews) do
            table.insert(sortedCrews, crewData)
        end

        table.sort(sortedCrews, function(a, b)
            return (a.points or 0) > (b.points or 0)
        end)

        local elements = {}

        local totalCrews = #sortedCrews
        table.insert(elements, {
            title = "Antal Crews: " .. tostring(totalCrews),
            description = "Tryk på et crew for medlemsliste!",
        })

        for i, crewData in ipairs(sortedCrews) do
            local kdRatio = "0.00"
            if crewData.deaths and crewData.deaths > 0 then
                kdRatio = string.format("%.2f", crewData.kills / crewData.deaths)
            elseif crewData.kills and crewData.kills > 0 then
                kdRatio = "∞"
            end

            table.insert(elements, {
                title = '#'..i..' | '..crewData.label..' ('..(crewData.tag or "N/A")..')',
                description = "Points: "..(crewData.points or 0) ..
                              " | Kills: " .. (crewData.kills or 0) ..
                              " | Deaths: " .. (crewData.deaths or 0) ..
                              " | K/D: "..kdRatio,
                onSelect = function()
                    crewLeaderboard.openMemberListForCrew(crewData)
                end
            })
        end
        ...
        lib.showContext('crew_leaderboard')
    end,

    openMemberListForCrew = function(crewData)
        local elements = {}
        local memberCount = getCrewMemberCount(crewData.data)
        table.insert(elements, {
            title = "Antal medlemmer: " .. tostring(memberCount),
        })
        
        for identifier, member in pairs(crewData.data) do
            table.insert(elements, {
                title = member.Name,
                icon = "user",
            })
        ...
        -- table.insert(elements, {
        --     title = "Tilbage",
        --     description = "Gå tilbage til leaderboardet",
        --     icon = "arrow-left",
        --     onSelect = function()
        --         lib.hideContext('crew_leaderboard_members')
        --         crewLeaderboard.openLeaderboard()
        --     end
        -- })
        lib.registerContext({
            id = 'crew_leaderboard_members',
            title = "Medlemsliste - " .. crewData.label,
            menu = 'crew_leaderboard',
            options = elements,
        })
        lib.showContext('crew_leaderboard_members')
    end
}

RegisterCommand('crewleaderboard', function()
    crewLeaderboard.openLeaderboard()
end)